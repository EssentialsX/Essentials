...
public class AnjoPermissionsHandler extends PermissionsReaderInterface {
    ...
    /**
     * Returns All permissions (including inheritance and sub groups) for the player, including child nodes from
     * Bukkit.
     *
     * @param userName
     *
    ...
    /**
     * Wrapper for offline server checks. Looks for the 'groupmanager.noofflineperms' permissions and reports no
     * permissions on servers set to offline.
     * <p/>
    ...
    public PermissionCheckResult checkFullGMPermission(User user, String targetPermission, Boolean checkBukkit) {
		 ...
		 */
        if (user == null || targetPermission == null || targetPermission.isEmpty() || (!Bukkit.getServer().getOnlineMode() && (checkPermission(user, "groupmanager.noofflineperms", false).resultType == PermissionCheckResult.Type.FOUND))) {
