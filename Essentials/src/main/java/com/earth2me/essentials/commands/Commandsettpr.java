...
public class Commandsettpr extends EssentialsCommand {
    ...
    protected void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        ...
        final boolean perWorld = randomTeleport.getPerWorld();
        if (perWorld && args.length < 1) {
            throw new Exception(tl("invalidWorld"));
        }
        final World world = perWorld ? Bukkit.getWorld(args[0]) : user.getWorld();
        ...
        final int argOffset = perWorld ? 1 : 0;
    ...
    protected List<String> getTabCompleteOptions(final Server server, final User user, final String commandLabel, final String[] args) {
        final boolean perWorld = ess.getRandomTeleport().getPerWorld();
        final int argOffset = perWorld ? 1 : 0;
