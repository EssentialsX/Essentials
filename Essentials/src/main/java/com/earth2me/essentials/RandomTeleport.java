...
public class RandomTeleport implements IConf {
    ...
    public Location getCenter(World world) {
        try {
            final Location center = config.getLocation(getWorldConfigKey(world, "center"), essentials.getServer());
            if (center != null) {
                return center;
            }
        } catch (final InvalidWorldException ignored) {
        }
        final World defaultWorld = world == null || !getPerWorld() ? essentials.getServer().getWorlds().get(0) : world;
        final Location center = defaultWorld.getWorldBorder().getCenter();
        center.setY(center.getWorld().getHighestBlockYAt(center) + HIGHEST_BLOCK_Y_OFFSET);
        setCenter(world, center);
        return center;
    }

    public void setCenter(final World world, final Location center) {
    ...
    public void setMinRange(final World world, final double minRange) {
    ...
    public void setMaxRange(final World world, final double maxRange) {
    ...
    public Queue<Location> getCachedLocations(Location center, double minRange, double maxRange) {
        final String cacheKey = String.valueOf(center.getWorld()) + minRange + maxRange;
    ...
    public CompletableFuture<Location> getRandomLocation(final Location center, final double minRange, final double maxRange) {
        ...
        final Queue<Location> cachedLocations = this.getCachedLocations(center, minRange, maxRange);
