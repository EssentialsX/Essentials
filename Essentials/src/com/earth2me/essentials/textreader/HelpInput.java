...
public class HelpInput implements IText {
    ...
    public HelpInput(final User user, final String match, final IEssentials ess) throws IOException {
        ...
        if (!match.equalsIgnoreCase("")) {
            lines.add(tlp(user, "helpMatching", match));
        }
        ...
        for (Plugin p : ess.getServer().getPluginManager().getPlugins()) {
            try {
                ...
                if (pluginNameLow.equals(match)) {
                    lines.clear();
                    newLines.clear();
                    lines.add(tlp(user, "helpFrom", p.getDescription().getName()));
                }
                    ...
                    try {
                        ...
                        if (pluginNameLow.contains("essentials")) {
                            ...
                            if (!ess.getSettings().isCommandDisabled(k.getKey()) && user.isAuthorized(node)) {
                                pluginLines.add(tlp(user, "helpLine", k.getKey(), k.getValue().get(DESCRIPTION)));
                            }
                        } else {
                            if (ess.getSettings().showNonEssCommandsInHelp()) {
                                ...
                                if (isOnWhitelist || user.isAuthorized("essentials.help." + pluginNameLow + "." + k.getKey())) {
                                    pluginLines.add(tlp(user, "helpLine", k.getKey(), value.get(DESCRIPTION)));
                ...
                if (!pluginLines.isEmpty()) {
                    ...
                    if (match.equalsIgnoreCase("")) {
                        lines.add(tlp(user, "helpPlugin", pluginName, pluginNameLow));
                    }
                }
            } catch (NullPointerException ex) {
            } catch (Exception ex) {
                if (!reported) {
                    logger.log(Level.WARNING, tlp(null, "commandHelpFailedForPlugin", pluginNameLow), ex);
                }
                reported = true;
            }
        }
