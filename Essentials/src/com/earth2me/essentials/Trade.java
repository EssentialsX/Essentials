...
public class Trade {
    ...
    public void isAffordableFor(final IUser user) throws ChargeException {
        ...
        if (getMoney() != null && getMoney().signum() > 0 && !user.canAfford(getMoney())) {
            throw new ChargeException(tlp(user, "notEnoughMoney", NumberUtil.displayCurrency(getMoney(), ess)));
        }

        if (getItemStack() != null && !user.getBase().getInventory().containsAtLeast(itemStack, itemStack.getAmount())) {
            throw new ChargeException(tlp(user, "missingItems", getItemStack().getAmount(), ess.getItemDb().name(getItemStack())));
        }

        BigDecimal money;
        if (command != null && !command.isEmpty() && (money = getCommandCost(user)).signum() > 0 && !user.canAfford(money)) {
            throw new ChargeException(tlp(user, "notEnoughMoney", NumberUtil.displayCurrency(money, ess)));
        }

        if (exp != null && exp > 0 && SetExpFix.getTotalExperience(user.getBase()) < exp) {
            throw new ChargeException(tlp(user, "notEnoughExperience"));
        }
    }
    ...
    public void charge(final IUser user) throws ChargeException {
        ...
        if (getItemStack() != null) {
            ...
            if (!user.getBase().getInventory().containsAtLeast(getItemStack(), getItemStack().getAmount())) {
                throw new ChargeException(tlp(user, "missingItems", getItemStack().getAmount(), getItemStack().getType().toString().toLowerCase(Locale.ENGLISH).replace("_", " ")));
            }
        ...
        if (command != null) {
            ...
            if (!user.canAfford(cost) && cost.signum() > 0) {
                throw new ChargeException(tlp(user, "notEnoughMoney", NumberUtil.displayCurrency(cost, ess)));
            }
            user.takeMoney(cost);
        }
        if (getExperience() != null) {
