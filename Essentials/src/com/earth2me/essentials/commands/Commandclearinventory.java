...
public class Commandclearinventory extends EssentialsCommand {
    ...
    private void parseCommand(Server server, CommandSource sender, String commandLabel, String[] args, boolean allowOthers, boolean allowAll)
        ...
        if (allowAll && args.length > 0 && args[0].contentEquals("*")) {
            sender.sendMessage(tlp(sender, "inventoryClearingFromAll"));
            offset = 1;
        ...
        if (senderUser != null && senderUser.isPromptingClearConfirm()) {
            if (!formattedCommand.equals(previousClearCommand)) {
                ...
                senderUser.sendMessage(tlp(senderUser, "confirmClear", formattedCommand));
                return;
            }
        }
    ...
    protected void clearHandler(CommandSource sender, Player player, String[] args, int offset, boolean showExtended) throws Exception {
        ...
        {
            if (showExtended) {
                sender.sendMessage(tlp(sender, "inventoryClearingAllItems", player.getDisplayName()));
            }
        ...
        {
            if (showExtended) {
                sender.sendMessage(tlp(sender, "inventoryClearingAllArmor", player.getDisplayName()));
            }
            InventoryWorkaround.clearInventoryNoArmor(player.getInventory());
            InventoryWorkaround.setItemInOffHand(player, null);
            ...
            if (data == -1) // data -1 means that all subtypes will be cleared
            {
                ...
                if (showExtended) {
                    sender.sendMessage(tlp(sender, "inventoryClearingAllStack", stack.getType().toString().toLowerCase(Locale.ENGLISH), player.getDisplayName()));
                }
            ...
            } else if (amount == -1) // amount -1 means all items will be cleared
            {
                ...
                if (removedAmount > 0 || showExtended) {
                    sender.sendMessage(tlp(sender, "inventoryClearingStack", removedAmount, stack.getType().toString().toLowerCase(Locale.ENGLISH), player.getDisplayName()));
                }
            } else {
                if (amount < 0) {
                    amount = 1;
                ...
                if (player.getInventory().containsAtLeast(stack, amount)) {
                    sender.sendMessage(tlp(sender, "inventoryClearingStack", amount, stack.getType().toString().toLowerCase(Locale.ENGLISH), player.getDisplayName()));
                    player.getInventory().removeItem(stack);
                } else {
                    if (showExtended) {
                        sender.sendMessage(tlp(sender, "inventoryClearFail", player.getDisplayName(), amount, stack.getType().toString().toLowerCase(Locale.ENGLISH)));
                    }
                }
            }
        }
